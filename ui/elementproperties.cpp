    ...
    , m_doc(doc)
{
    ...
    // Connections
    connect(idLineEdit, &QLineEdit::editingFinished,
            this, &ElementProperties::onIdEditingFinished);
    connect(zOrderSpinBox, QOverload<int>::of(&QSpinBox::valueChanged),
            this, &ElementProperties::onZOrderChanged);
    connect(opacitySpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            this, &ElementProperties::onOpacityChanged);
    ...
    connect(animateIn,  &AnimationEditor::animationChanged, this, &ElementProperties::onAnimInChanged);
    connect(animateOut, &AnimationEditor::animationChanged, this, &ElementProperties::onAnimOutChanged);

    connect(maskComboBox,   QOverload<int>::of(&QComboBox::currentIndexChanged),
...
void ElementProperties::onDocumentChanged()
{
    ...
    idLineEdit->setStyleSheet(usedAsParent
        ? "QLineEdit { color: #888888; font-style: italic; }"
        : "");
    idLineEdit->setToolTip(usedAsParent
...
void ElementProperties::onParentChanged(int)
{
    ...
    try {
        ...
        if (!newParentId.empty()) {
            try {
                Point parentGlobal = m_doc->scene()
                    .GetById(m_graphicId).GetById(newParentId).GetGlobalPosition();
